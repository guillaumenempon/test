...
RUN export DEBIAN_FRONTEND=noninteractive \
  ...
  && rm /opt/adt.tgz \
  && ( sleep 5 && while [ 1 ]; do sleep 1; echo y; done ) | /opt/android-sdk-linux/tools/android update sdk   -a -u --filter android-${ANDROID_COMPILE_SDK},platform,platform-tools,build-tools-${ANDROID_BUILD_TOOLS} \
  && apt-get -qqy clean \
  && rm -rf /var/cache/apt/*
...
RUN mkdir /opt/appium \
  ...
  && mkdir /opt/android-sdk-linux/build-tools 

#==========================
# Run appium as default
#==========================
CMD ["sleep", "infinity"]
#CMD appium --port 4723 --port 4725 
#CMD adb devices
